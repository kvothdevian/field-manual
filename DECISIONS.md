...
| 19 | 2026-08-26 | FINAL-FORM ALIGNMENT: the deliverable is a SINGULAR self-contained HTML book. Multi-page sources stay for development; scripts/build-book.mjs compiles all chapters (TOC order) + inlined CSS/JS into one offline book.html at every milestone. All build repos consolidated under field-manual/builds/ (own git preserved, parent ignores them) - one folder, no sprawl | Per Aman: singular html book, single folder; supersedes the multi-page-only reading of #4 |
...
| 24 | 2026-09-04 | V2 CUTOVER: v2/ moves to repo root and replaces the v1 page tree at the Pages URL; v1 preserved on git tag `v1-final`. `book.html` stays a local-only git-ignored artifact (corrects #19 to match what was ever actually deployed: the multi-page site). v2 has no search yet — first fast-follow | Per Aman: ship v2 where v1 was hosted, no stale v1 files |

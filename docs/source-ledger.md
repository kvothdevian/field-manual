...
| Anthropic context engineering (attention budget, n², just-in-time retrieval, progressive disclosure, compaction/note-taking/subagents triad, smallest high-signal set) | https://anthropic.com/engineering/effective-context-engineering-for-ai-agents | T1 | 2026-09-03 | y | v2 ch05: fetched live 2026-09-03; defining essay re-verified |
| opencode stats as audit instrument: sessions/messages/cost/avg+median tokens per session (median 356K here) | https://opencode.ai/docs | T1 | 2026-09-03 | y | v2 ch05: `opencode stats` executed locally; exact figures 2026-09-03 — 168 sessions, 4,483 messages, $0.00 total, avg 2.6M/session, median 356.2K/session, 14 days; audit step built on it |
| Compaction detail (same essay): agent continues with compressed context PLUS the five most recently accessed files | https://anthropic.com/engineering/effective-context-engineering-for-ai-agents | T1 | 2026-09-03 | y | v2 ch05 round-2: quoted from fetched essay for step-4 bar |
| ch05 trim 2026-09-03: memory 1,122B→100B (91% cut, ~280→~25 tokens), re-run fixed +1/-1, proof clean 12 with no junk obedience (vs ZEBRA-7 under bloat) | https://research.trychroma.com/context-rot | T2 | 2026-09-03 | y | v2 ch05: own-execution; ≥30% trim demonstrated by re-run per teach test |
| ch05 memory sizes measured 2026-09-03 (context-lab): slim AGENTS.md 66B, bloated 1,122B, trimmed 100B — A/B pair 66B vs 1,122B, trim pair 1,122B→100B | https://research.trychroma.com/context-rot | T2 | 2026-09-03 | y | v2 ch05 round-2: byte counts measured live (slim via committed file, bloated/trimmed via getsize); chapter figures pinned here |
| opencode session list as stats fallback (executed 2026-09-03, lists session IDs/titles/updated) | https://opencode.ai/docs | T1 | 2026-09-03 | y | v2 ch05: local output captured for step-1 stuck fallback |
| ch05 A/B 2026-09-03: slim AGENTS.md run (direct read, +1/-1) vs bloated run (extra reads, ZEBRA-7 junk obeyed, false no-change narrative over a real +1/-1 diff) | https://research.trychroma.com/context-rot | T2 | 2026-09-03 | y | v2 ch05: own-execution pair; bloat effects observed, mechanisms labeled as observed-not-proven |
| ch05 delegation 2026-09-03: Explore subagent spawned, reported accurately (incl. committed-vs-working-tree state), self-priced 6-8x overhead vs direct read on the tiny repo | https://cognition.ai/blog/dont-build-multi-agents | T2 | 2026-09-03 | y | v2 ch05: own-execution; delegate-where-it-pays demonstrated with numbers |
...
| Spec Kit re-verified: 1.0.0 shipped, 133k stars; dual command forms (/speckit.* slash + speckit-* skills); flow adds converge + clarify/analyze/checklist/taskstoissues; bug/assess extensions; specify-cli via uv | https://github.com/github/spec-kit | T1 | 2026-09-03 | y | v2 ch06: fetched live; install needs uv+Python (not installed here — reader-run); present both command forms |
| Five-shape taxonomy URL-form row: chaining/routing/parallelization/orchestrator-workers/evaluator-optimizer + simplest-first + complexity-only-when-measured | https://www.anthropic.com/research/building-effective-agents | T1 | 2026-09-03 | n | v2 ch06 round-2: URL-form row for closed-world matching; substance per Aug bare-form row |
| Plan-first mechanics URL-form row: read-only plan, approve before edits, delegate research to subagents | https://code.claude.com/docs/en/common-workflows | T1 | 2026-09-03 | n | v2 ch06 round-2: URL-form row for closed-world matching; substance per Aug bare-form row |
| Spec Kit sub-steps (same live README): bug extension assess→fix→test; assess extension intake→research→define→shape→decide; specify excludes tech, plan owns it | https://github.com/github/spec-kit | T1 | 2026-09-03 | y | v2 ch06 round-2: quoted from fetched README for §ii depth |
| Routing maintenance + isolation interfaces (analysis backed by taxonomy + counterpoint rows): fallback routes + confusion logs; owned files + return schemas + conflict rules; correlated-blindness voting caveat labeled analysis | https://cognition.ai/blog/dont-build-multi-agents | T2 | 2026-09-03 | y | v2 ch06 round-2: prescription rows for §ii workshop rules; correlated-blindness fenced as disprovable analysis |
| opencode plan Tab URL-form row: Tab flips plan (read-only) / build (full access) | https://opencode.ai/docs | T1 | 2026-09-03 | y | v2 ch06 round-2: URL-form row for closed-world matching; executed ch01/ch02 |
...
| Reflexion v2 URL-form row (substance unchanged): reflection loop + expired digits, honestly labeled craft cousin | https://arxiv.org/abs/2303.11366 | T1 | 2026-09-03 | n | v2 ch06: URL-form row for closed-world matching; see Aug row for substance |
...
| Red-green re-verified: test-first + confirmed-failure + red/green shorthand + example prompt (page created Feb 23, modified Feb 28 2026) | https://simonwillison.net/guides/agentic-engineering-patterns/red-green-tdd/ | T1 | 2026-09-03 | y | v2 ch06: fetched live; content holds |
| ch06 three-way 2026-09-03 (median task): direct green +3/-1; plan-only honored (read-only, 3-step plan); plan-implement green +3/-1; evaluator critic 3/3 green but named untested even-length branch as biggest gap | https://www.anthropic.com/research/building-effective-agents | T1 | 2026-09-03 | y | v2 ch06: own-execution transcripts of direct/plan-first/evaluator shapes; critic finding genuine (builder over-built past the suite) |
| ch06 TDD red demonstrated: median.test.js failed 1/2 before any fix (median-of-three red), green after | https://simonwillison.net/guides/agentic-engineering-patterns/red-green-tdd/ | T1 | 2026-09-03 | y | v2 ch06: own-execution; confirmed-failure step captured |
